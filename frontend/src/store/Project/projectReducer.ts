...
import { emptyTaxonomy, resolveLabelDisplayName } from "../../utils/taxonomy";
...
export type ProjectAction =
	...
	| {
			type: "SET_LABEL_TYPE";
			payload: { labelId: number; labelType: LabelType };
	  }
	| {
			type: "SET_LABEL_TAXONOMY";
			payload: { labelId: number; taxonomy: Taxonomy };
	  }
	| {
	...
	| {
			...
			payload: { dataId: number; lineId: number };
	  }
	| {
			type: "SET_CORAL_WATCH_CARD";
			payload: { dataId: number; card: CoralWatchCard | null };
	  }
	| { type: "EXCLUDE_LABEL"; payload: { labelId: number } }
	| { type: "INCLUDE_LABEL"; payload: { labelId: number } }
	| { type: "SET_EXCLUDED_LABELS"; payload: { labelIds: number[] } };

function syncLabelNameWithTaxonomy(label: Label): Label {
	if (!label.taxonomy) return label;
	const resolvedName = resolveLabelDisplayName(label);
	return resolvedName === label.name ? label : { ...label, name: resolvedName };
}
	...
	return {
		...
		dataList: state.dataList.map((data) =>
			data.id === dataId
				? {
						...data,
						annotations: [...data.annotations, ...newAnnotations],
					}
				: data,
		),
	};
...
function addLabels(
	state: ProjectState,
	labelsToAdd: Array<Omit<Label, "id">>,
): ProjectState {
	if (labelsToAdd.length === 0) return state;

	const nextLabels = [...state.labels];
	for (const imported of labelsToAdd) {
		nextLabels.push(
			syncLabelNameWithTaxonomy({
				...imported,
				id: nextLabels.length,
			}),
		);
	}
...
function deleteLabel(state: ProjectState, labelId: number): ProjectState {
	...
	return {
		...
		excludedLabelIds: state.excludedLabelIds
...
function setLabelType(
	...
	return {
		...
		labels: state.labels.map((label) => {
			...
			// Seed an empty taxonomy when becoming taxonomic for the first time;
			// keep any existing taxonomy so toggling back and forth is lossless.
			const taxonomy =
				labelType === "taxonomic"
					? (label.taxonomy ?? emptyTaxonomy())
					: label.taxonomy;
			return syncLabelNameWithTaxonomy({ ...label, type: labelType, taxonomy });
		}),
	};
}

function setLabelTaxonomy(
	state: ProjectState,
	labelId: number,
	taxonomy: Taxonomy,
): ProjectState {
...
function excludeLabel(state: ProjectState, labelId: number): ProjectState {
	if (state.excludedLabelIds.includes(labelId)) return state;
	return {
		...state,
		excludedLabelIds: [...state.excludedLabelIds, labelId],
	};
}

function includeLabel(state: ProjectState, labelId: number): ProjectState {
	return {
		...state,
		excludedLabelIds: state.excludedLabelIds.filter((id) => id !== labelId),
	};
}

function setExcludedLabels(
	state: ProjectState,
	labelIds: number[],
): ProjectState {
	return {
		...state,
		excludedLabelIds: labelIds,
	};
}
...
export function projectReducer(
	...
	switch (action.type) {
		case "LOAD_PROJECT":
			return {
				...action.payload,
				labels: action.payload.labels.map(syncLabelNameWithTaxonomy),
			};
		...
		case "ADD_LABELS":
			return addLabels(state, action.payload.labels);
		case "DELETE_LABEL":
		...
		case "SET_LABEL_TYPE":
			return setLabelType(
				state,
				action.payload.labelId,
				action.payload.labelType,
			);
		case "SET_LABEL_TAXONOMY":
			return setLabelTaxonomy(
				state,
				action.payload.labelId,
		...
		case "EXCLUDE_LABEL":
			return excludeLabel(state, action.payload.labelId);
		case "INCLUDE_LABEL":
			return includeLabel(state, action.payload.labelId);
		case "SET_EXCLUDED_LABELS":
			return setExcludedLabels(state, action.payload.labelIds);
		default:
			return state;
	}
}
